...
namespace ts.codefix {
    ...
    function tryGenerateTypes(changes: textChanges.ChangeTracker, packageName: string, context: CodeFixContextBase): GenerateTypesAction | undefined {
        ...
        const outputFileName = resolvePath(getDirectoryPath(configFile.fileName), typesDir, packageName + ".d.ts");
    ...
    function getOrCreateTypesDirectory(tsconfig: TsConfigSourceFile, changes: textChanges.ChangeTracker): string | undefined {
        ...
        if (!compilerOptionsProperty) {
            ...
            changes.insertNodeAtObjectStart(tsconfig, tsconfigObjectLiteral, createJsonPropertyAssignment("compilerOptions", newCompilerOptions));
        ...
        const baseUrl = getOrAddBaseUrl(changes, tsconfig, compilerOptions);
        const typesDirectoryFromPathMapping = getOrAddPathMapping(changes, tsconfig, compilerOptions);
